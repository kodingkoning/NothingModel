...
to cluster-setup
  let group_x random-xcor
  let group_y random-ycor
  create-turtles count patches [
    setxy group_x group_y
  ]
  ask turtles [
...
to prob-go
  ask patches [
    if random-float 1 < action-prob [ ; computation
      computation_rng
      computation_mult
      computation_factors
    ]
    if random-float 1 < action-prob [ ; communication
      all-to-all-comm
      radius-comm
      pair-comm-fixed
      pair-comm-rand
